...
$$
\widetilde{\mathbf{A}} = \mathbf{D}^{-1/2} \mathbf{A} \mathbf{D}^{-1/2} = \begin{bmatrix}
...
$$
\widetilde{\mathbf{A}} \approx \begin{bmatrix}
0.333 & 0.333 & 0.333 \\\\
0.333 & 0.333 & 0.333 \\\\
0.333 & 0.333 & 0.333
\end{bmatrix}
...
### 5.2 Step-by-Step Message Calculation for Edge B → A (m_AB)

#### Step 1: Concatenation [x_A || x_B]

$$
\mathbf{x}_{\text{edge}, AB} = \begin{bmatrix}
-0.213946 \\
-0.054820 \\
-0.303199 \\
+4.197146 \\
...
#### Step 2: First Linear Transformation ($\mathbf{u} = \mathbf{W}_1 \mathbf{x} + \mathbf{b}_1$)

* $u_1 = 0.2(-0.214) - 0.1(-0.055) + 0.3(-0.303) + 0.1(4.197) + 0.4(0.179) - 0.2(-0.842) + 0.10 = \mathbf{+0.702580}$
* $u_2 = -0.3(-0.214) + 0.2(-0.055) + 0.1(-0.303) - 0.2(4.197) + 0.1(0.179) + 0.3(-0.842) - 0.05 = \mathbf{-1.100913}$
* $u_3 = 0.1(-0.214) + 0.3(-0.055) - 0.2(-0.303) + 0.3(4.197) - 0.1(0.179) + 0.2(-0.842) + 0.05 = \mathbf{+1.151745}$
* $u_4 = -0.2(-0.214) + 0.1(-0.055) + 0.4(-0.303) - 0.1(4.197) + 0.2(0.179) + 0.1(-0.842) + 0.00 = \mathbf{-0.551065}$
...
Equation: $\mathbf{h} = \max(0, \mathbf{u})$
...
Equation: $\mathbf{m}_{AB} = \mathbf{W}_2 \mathbf{h}$
...
| Message | Source $\to$ Target | Resulting Vector $\mathbf{m}_{ij}^T$ |
|:---:|:---:|:---:|
| $\mathbf{m}_{AA}$ | $A \to A$ (Self-loop) | $[+0.000000,\; +0.000000]$ |
| $\mathbf{m}_{AB}$ | $B \to A$ | $[+0.481633,\; -0.011668]$ |
| $\mathbf{m}_{AC}$ | $C \to A$ | $[+0.156264,\; +0.043982]$ |
| $\mathbf{m}_{BA}$ | $A \to B$ | $[+0.389007,\; -0.080047]$ |
| $\mathbf{m}_{BB}$ | $B \to B$ (Self-loop) | $[+0.884328,\; -0.089251]$ |
| $\mathbf{m}_{BC}$ | $C \to B$ | $[+0.558959,\; -0.033601]$ |
| $\mathbf{m}_{CA}$ | $A \to C$ | $[+0.262523,\; -0.135668]$ |
| $\mathbf{m}_{CB}$ | $B \to C$ | $[+0.735277,\; -0.126817]$ |
| $\mathbf{m}_{CC}$ | $C \to C$ (Self-loop) | $[+0.444343,\; -0.098715]$ |
...
### 6.3 Spatial Node Embedding for Node A (X_t, A)

With convolution layer bias $\mathbf{b}_{\text{conv}} = [0, 0]^T$:

$$
\mathbf{X}_{t, A} = \mathbf{aggr}_A + \mathbf{skip}_A + \mathbf{b}_{\text{conv}}
$$

$$
\mathbf{X}_{t, A} = \begin{bmatrix} +0.212632 \\ +0.010772 \end{bmatrix} + \begin{bmatrix} +0.040974 \\ -0.141493 \end{bmatrix} = \begin{bmatrix} \mathbf{+0.253606} \\ \mathbf{-0.130721} \end{bmatrix}
$$
...
$$
\mathbf{H}_{t-1, A} = \begin{bmatrix} +0.100000 \\ -0.100000 \end{bmatrix}, \quad \mathbf{X}_{t, A} = \begin{bmatrix} +0.253606 \\ -0.130721 \end{bmatrix}
$$

$$
\mathbf{c}_A = [\mathbf{X}_{t, A} \,\|\, \mathbf{H}_{t-1, A}] = \begin{bmatrix} +0.253606 \\ -0.130721 \\ +0.100000 \\ -0.100000 \end{bmatrix}
$$

#### 1. Update Gate

Formula: $\mathbf{Z}_t = \sigma(\mathbf{W}_z \mathbf{c}_A + \mathbf{b}_z)$
...
Formula: $\mathbf{R}_t = \sigma(\mathbf{W}_r \mathbf{c}_A + \mathbf{b}_r)$
...
$$
\mathbf{r}_{\text{mem}} = \mathbf{R}_t \odot \mathbf{H}_{t-1, A}
$$
...
$$
\widetilde{\mathbf{H}}_t = \tanh(\mathbf{W}_h [\mathbf{X}_{t, A} \,\|\, \mathbf{r}_{\text{mem}}] + \mathbf{b}_h)
$$

* $a_{h1} = 0.5(0.254) - 0.1(-0.131) + 0.3(0.049) - 0.2(-0.052) + 0.05 = +0.214954$  
  $\implies \widetilde{H}_{t, 1} = \tanh(0.214954) = \mathbf{+0.211716}$

* $a_{h2} = -0.2(0.254) + 0.4(-0.131) + 0.1(0.049) + 0.5(-0.052) - 0.05 = -0.174116$  
  $\implies \widetilde{H}_{t, 2} = \tanh(-0.174116) = \mathbf{-0.172385}$
...
$$
\mathbf{H}_t = \mathbf{Z}_t \odot \mathbf{H}_{t-1, A} + (1 - \mathbf{Z}_t) \odot \widetilde{\mathbf{H}}_t
$$
...
$$
\hat{\text{Units}} = \hat{y}_{\text{norm}, 0} \times \sigma_{\text{units}} + \mu_{\text{units}}
$$

$$
\hat{\text{Load}} = \hat{y}_{\text{norm}, 1} \times \sigma_{\text{load}} + \mu_{\text{load}}
$$

### 9.1 Calculation for Node A

$$
\hat{\text{Units}}_A = (-0.087280) \times 19467.2871 + 12823.9473 = \mathbf{11{,}124.84\text{ kWh}}
$$

$$
\hat{\text{Load}}_A = (+0.081213) \times 1962.5702 + 435.5880 = \mathbf{594.97\text{ kW}}
$$
